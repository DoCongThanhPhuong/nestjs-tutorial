...
export class SubmissionsService {
  ...
  async submitForm(
    ...
    if (!this.usersService.canSubmitForm(user, form.formType.scope)) {
      throw new NotFoundException('This form type is not for the current user');
    }
  ...
  async approveSubmissionById(
    ...
    if (managerId && submission.managerId !== managerId)
      throw new NotFoundException('You are not the direct manager');
  ...
  async reportFormSubmissions(formId: number) {
    ...
    const employees = await this.usersService.getAllUserIds(isOfficial);
